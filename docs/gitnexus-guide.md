...
_(Nếu muốn giữ lại các vector embeddings đã tạo, nhớ thêm `--embeddings`)_.
...
1. **Chạy thử để xem trước các thay đổi (Dry run)**:
   ...
   ```javascript
   gitnexus_rename({
     symbol_name: "generatePodcast",
     new_name: "createAudioSummary",
     dry_run: true,
   });
   ```
...
**\*Lưu ý:** Hiện tại MCP GitNexus chưa được liên kết với hệ thống của tôi (tôi chưa thể tự động gọi các hàm mcp\_... trên). Bạn cần cấu hình tool GitNexus trong IDE (như hướng dẫn ở phản hồi trước) trước khi yêu cầu tôi tự động gọi nó.\*
